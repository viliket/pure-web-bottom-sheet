...
describe("Non-dismissible bottom sheet", function () {
  ...
  beforeEach(async function () {
    ...
    if (browserName === "MiniBrowser") {
      // MiniBrowser has occasional issues with the initial snap on page load,
      // refreshing to ensure the correct initial state
  ...
  it("should not block scroll interaction with the page behind the sheet", async function () {
    if (browserName === "MiniBrowser") {
      // MiniBrowser has issues with wheel actions not behaving as expected, possibly
      // related to its WebDriver implementation, so skipping this test for now.
      this.skip();
    }
