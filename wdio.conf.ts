...
export const config: Options.Testrunner & {
  ...
  capabilities: [
    ...
    ...when<WebdriverIO.Capabilities>(browsers.has("firefox"), {
      ...
      webSocketUrl: true,
      "moz:firefoxOptions": {
        args: [...when(headless, "-headless"), "-width=1280", "-height=800"],
      },
    }),
    ...
    ...when<WebdriverIO.Capabilities>(browsers.has("minibrowser"), {
      ...
      // MiniBrowser only supports a single session — group all specs into one
      // worker so they reuse the same browser instance instead of closing and
      // re-creating it for each spec file.
      "wdio:maxInstances": 1,
      specs: [
